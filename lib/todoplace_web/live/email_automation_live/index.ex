defmodule TodoplaceWeb.Live.EmailAutomations.Index do
  ...
  defp pipeline_section(assigns) do
    ~H"""
    <section
      class="mx-auto border border-base-200 rounded-lg mt-2 overflow-hidden"
      testid={@pipeline.state}
    >
      <div
        class="flex justify-between bg-base-200 pl-4 pr-7 py-3 items-center cursor-pointer"
        phx-click="toggle-section"
        phx-value-section_id={"pipeline-#{@pipeline.id}"}
      >
          ...
          <div class="flex flex-col">
            <span class="text-blue-planning-300 text-xl font-bold ml-3">
              ...
              <span class="text-base-300 ml-2 rounded-md bg-white px-2 text-sm font-bold whitespace-nowrap">
                <%= Enum.count(@pipeline.emails) %> <%= ngettext(
                  "email",
                  "emails",
                  Enum.count(@pipeline.emails)
                ) %>
              </span>
              <%= if is_state_manually_trigger(@pipeline.state) do %>
                <span class="text-base-300 ml-2 rounded-md bg-white px-2 text-sm font-bold whitespace-nowrap">
      ...
      <%= if Enum.member?(@collapsed_sections, "pipeline-#{@pipeline.id}") do %>
                  ...
                  >
                    <%= cond do %>
                      ...
                      <% true -> %>
                        <.icon
                          name="close-x"
                          class="w-4 h-4 stroke-current stroke-3 text-blue-planning-300"
                        />
                    <% end %>
                  </div>
                </div>
                <div class="ml-3 py-6">
                  <div class="text-xl font-bold">
                    ...
                    <%= if email.status == :disabled do %>
                      <span class="ml-2 rounded-md bg-red-sales-100 text-red-sales-300 px-2 text-sm font-bold whitespace-nowrap">
                        Disabled
                      </span>
                    <% end %>
                  </div>
              ...
              <div class="flex items-center md:mt-0 ml-auto md:pb-0 pb-6 md:pt-6">
                <div class="custom-tooltip">
                  <%= unless emails_send_immediately_states?(@subcategory_slug, @pipeline.state) do %>
                    <.icon_button
                      id={"email-#{email.id}"}
                      disabled={disabled_email?(index)}
                      class="ml-8 mr-2 px-2 py-2"
                      title={!(index === 0) && "remove"}
                  ...
                  <%= if index == 0 do %>
                    <span
                      class={
                        classes("text-black font-normal w-64 text-start", %{
                          " !-left-20" => is_state_manually_trigger(@pipeline.state)
                        })
                      }
                ...
                <%= unless emails_send_immediately_states?(@subcategory_slug, @pipeline.state) do %>
                  <.icon_button_simple
                    class={
                      classes(
                        "flex items-center px-2 py-1 btn-tertiary text-blue-planning-300 hover:border-blue-planning-300 mr-2 whitespace-nowrap",
                        %{"hidden" => is_state_manually_trigger(@pipeline.state) and index == 0}
                      )
                    }
                    phx-click="edit-time-popup"
                    phx-value-index={index}
                    phx-value-subcategory_slug={@subcategory_slug}
                    phx-value-email_id={email.id}
                ...
                <% end %>
                <.icon_button_simple
                  class="flex items-center px-2 py-1 btn-tertiary bg-blue-planning-300 text-white hover:bg-blue-planning-300/75 whitespace-nowrap"
                  phx-click="edit-email-popup"
                  phx-value-index={index}
                  phx-value-email_id={email.id}
                  phx-value-pipeline_id={@pipeline.id}
                  icon_class="inline-block w-4 h-4 mr-3"
        ...
        <div class="flex flex-row justify-between pr-6 pl-8 sm:pl-16 py-6">
          <div class="flex items-center">
            <%= unless emails_send_immediately_states?(@subcategory_slug, @pipeline.state) do %>
              <.icon_button_simple
                class="flex items-center px-2 py-1 btn-tertiary hover:border-blue-planning-300"
                phx-click="add-email-popup"
                phx-value-pipeline_id={@pipeline.id}
          ...
          <%= if show_enable_setting?(@pipeline.state, @subcategory_slug) do %>
            <div class="flex flex-row">
              <.form
                ...
                phx-click="toggle"
                phx-value-pipeline_id={@pipeline.id}
                phx-value-active={is_pipeline_active?(@pipeline.status) |> to_string}
              >
                <label class="flex">
                  <input
                    type="checkbox"
                    class="peer hidden"
                    id={"sequence-toggle-#{@subcategory_slug}"}
                    checked={is_pipeline_active?(@pipeline.status)}
                  />
                  <div
                    testid={"enable-#{@pipeline.id}"}
                    class="hidden peer-checked:flex cursor-pointer"
                  >
                  ...
                  <div
                    testid={"disable-#{@pipeline.id}"}
                    class="flex peer-checked:hidden cursor-pointer"
                  >
