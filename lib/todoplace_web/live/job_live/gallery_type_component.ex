defmodule TodoplaceWeb.JobLive.GalleryTypeComponent do
  ...
  def render(assigns) do
    ...
    <div class={"#{@main_class} items-center mx-auto bg-white relative"}>
      ...
      <h1 class={classes("font-bold text-3xl mb-8", %{"hidden" => !@from_job?})}>
        Set Up Your Gallery
      </h1>
      <.card
  ...
  def card(assigns) do
    ...
    <div class={"border hover:cursor-pointer hover:border-#{@color} h-full my-3 rounded-lg bg-#{@color} overflow-hidden"}>
      <div class="h-full p-8 bg-white flex items-center ml-3">
        <.icon
          name={@icon}
          class="w-11 h-11 inline-block mr-2 rounded-sm fill-current text-blue-planning-300"
        />
        ...
        <button
          class={"#{@button_class} px-9 ml-auto"}
          phx-value-type={@type}
          phx-click="gallery_type"
          phx-target={@target}
