defmodule TodoplaceWeb.Calendar.Shared.DetailComponent do
  ...
  def section(%{opts: %{calender: "internal"}} = assigns) do
    ...
    <div class="pt-4 flex flex-col">
      ...
      <%= if @opts.location && @opts.address do %>
        <.event_item
          icon="vector"
          item_title={@opts.address}
          item_desc={to_string(@opts.location) |> String.replace("_", " ") |> String.capitalize()}
        />
      <% end %>
      ...
      <.event_item
        icon="calendar"
        icon_class="mt-1"
        item_title="Todoplace Calendar"
        item_desc="Created by: Todoplace"
      />
    </div>
  ...
  def section(%{opts: %{calender: "external"}} = assigns) do
    ...
    <div class="pt-4 flex flex-col">
      ...
      <.event_item
        icon="calendar"
        item_title="Home Calendar"
        item_desc={"Created by: #{@opts.organizer_email}"}
      />
    </div>
    """
  end
