    ...
    <div class={"flex items-center mt-4 md:justify-start #{@class}"}>
      <div class={classes("flex items-center", %{"flex-col lg:block" => @input_label})}>
        <.link navigate={@back_path}>
          <div class={
            classes("flex items-center justify-center rounded-full p-2.5 mt-2 mr-4 bg-base-200", %{
              "lg:hidden shadow-lg bg-base-100 mt-0 px-4 py-3 w-fit mb-2" => @input_label
            })
          }>
            <.icon
              name="back"
              class={
                classes("stroke-current w-4 h-4 stroke-2", %{
                  "w-2 h-3 stroke-3 text-blue-planning-300" => @input_label
                })
              }
            />
            <%= if @input_label do %>
          ...
          <div class={classes("flex #{@main_class}", %{"hidden" => @edit_name})}>
            ...
            <%= if @input_label do %>
              <.icon_button
                ...
                phx-click="click"
                phx-target={@myself}
                class="bg-white px-2 py-1 pt-2 pb-2 mt-4 shadow-lg w-fit"
            ...
            <% else %>
              <.icon_button
                phx-click="click"
                phx-target={@myself}
                class="bg-gray-200 mt-3 px-2 py-1 shadow-lg w-fit"
                color="blue-planning-300"
                icon="pencil"
              />
            <% end %>
          ...
          <div class={classes("flex", %{"hidden" => !@edit_name, "flex-col" => @input_label})}>
            ...
            <div class="flex gap-2">
              <%= submit("Save",
                ...
                class:
                  "grid place-items-center ml-4 px-4 py-2 border rounded-lg shadow-lg hover:opacity-75 border-blue-planning-300"
              ) %>
              <button
                class="grid place-items-center p-2 border rounded-lg shadow-lg hover:opacity-75 border-black"
                title="cancel"
                type="button"
                phx-click="close"
                phx-target={@myself}
