defmodule TodoplaceWeb.Live.Pricing.Category.Variation do
    ...
    <div class={"contents #{if @expanded, do: "expanded", else: "collapsed"}"}>
      <%= if @expanded do %>
        <button
          type="button"
          title="Expand"
          class="flex items-center p-4 text-xl font-bold rounded-lg sm:text-base sm:col-span-5 col-span-2 pointer bg-blue-planning-300 text-base-100"
      ...
      <% else %>
        <button
          type="button"
          title="Expand"
          class="flex items-center p-4 text-xl font-bold col-start-1 sm:text-base"
          phx-click="toggle-expand"
          phx-target={@myself}
        >
          <.icon
            name="down"
            class="w-4 h-2 mr-12 stroke-current sm:mr-4 stroke-3 text-blue-planning-300"
          />
          <%= @variation.name %>
        </button>

        <div class="items-center hidden p-4 sm:flex text-base-250">
          From <%= min_base_price(@variation) %>
        </div>
        <div class="items-center hidden p-4 sm:flex text-base-250">
          From <%= final_price(@variation) %>
        </div>
        <div class="flex items-center text-lg col-start-2 sm:text-base sm:col-start-4 pl-14 sm:p-4 text-base-250">
          From <%= profit(@variation) %>
        </div>
        <div class="items-center hidden p-4 sm:flex text-base-250">
          From <%= markup(@variation) %>
        </div>
      <% end %>
    </div>
    """
  ...
  defp attribute(assigns) do
    ~H"""
    <.live_component
      module={TodoplaceWeb.Live.Pricing.Category.Attribute}
      attribute={@attribute}
      variation_id={@variation_id}
      id={Enum.join([@variation_id, @attribute.category_id, @attribute.id], "-")}
      update={@update}
    />
    """
  end
