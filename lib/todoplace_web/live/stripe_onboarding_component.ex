      ...
      <.form :let={_} for={%{}} as={:stripe} phx-submit="link-stripe" phx-target={@myself}>
        <%= case @stripe_status do %>
          ...
          <% :error -> %>
            <button
              type="submit"
              phx-disable-with="Retry Stripe account"
              class={@class}
              {add_card_hook(@org_card_id)}
            >
              Retry Stripe account
            </button>
            <em class={"block pt-1 text-xs text-red-sales-300 " <> @error_class}>
              Error accessing your Stripe information.
            </em>
          <% :no_account -> %>
            <button
              type="submit"
              phx-disable-with="Set up Stripe"
              class={@class}
              {add_card_hook(@org_card_id)}
            >
              Set up Stripe
          ...
          <% :missing_information -> %>
            <button
              type="submit"
              phx-disable-with="Stripe Account incomplete"
              class={@class}
              {add_card_hook(@org_card_id)}
            >
              Stripe Account incomplete
            </button>
            <em class="block pt-1 text-xs text-center text-red-sales-300">
              Please provide missing information.
            </em>
          <% :pending_verification -> %>
            <button
              type="submit"
              phx-disable-with="Check Stripe status"
              class={@class}
              {add_card_hook(@org_card_id)}
            >
              Check Stripe status
            </button>
            <em class="block pt-1 text-xs text-center">
              Your account has been created. Please wait for Stripe to verify your information.
            </em>
          <% :charges_enabled -> %>
            <a
