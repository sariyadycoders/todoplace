defmodule TodoplaceWeb.UserRegisterLive do
  ...
  defp onboarding_view(%{onboarding_type: nil} = assigns) do
    ...
    <.optimized_container step={@step} steps={@steps} color_class="bg-blue-planning-200">
      <.signup_hooks />
      <h1 class="text-3xl font-bold sm:leading-tight mt-2">
        <%= @subscription_plan_metadata.signup_title %>
      </h1>
      <h2 class="text-base mt-4 font-normal">
        <%= @subscription_plan_metadata.signup_description %> or <%= link("log in",
          to: ~p"/users/log_in",
  ...
  defp onboarding_view(%{onboarding_type: "mastermind"} = assigns) do
    ...
    <.signup_container
      {assigns}
      step={1}
      step_total={length(@steps)}
      step_title="Let’s get to know you"
      left_classes="p-8 pb-0 bg-purple-marketing-300 text-white flex flex-col"
    >
      <h2 class="text-3xl md:text-3xl font-bold text-center mb-2">
        Gift Yourself Success and Support!
      </h2>
      <h3 class="text-xl text-center mb-4 italic">
        Sign up and get a full year of Todoplace for $350
      </h3>
      <p class="text-center">
        Make the switch to Todoplace in 2024—get the software, support, and education you need to grow your business!
      </p>
      <ul class="mb-8 space-y-2">
        <li class="flex gap-2 font-bold font-italic">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          One-year of Todoplace’s all-in-one software
        </li>
        <li class="flex gap-2">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          12-months of the Business Mastermind
        </li>
        <li class="flex gap-2">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          Best-in-the- business customer support
        </li>
        <li class="flex gap-2">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          Resources that will help you market, manage, and monetize your business
        </li>
      </ul>
      <img
        class="mt-auto"
        src={static_path(@socket, "/images/mastermind-hero.png")}
        loading="lazy"
        alt="Images of the Todoplace App"
      />
      <:right_panel>
        <.signup_hooks />
  ...
  defp onboarding_view(%{onboarding_type: "three_month"} = assigns) do
    ...
    <.signup_container
      ...
      step_total={length(@steps)}
      step_title="Let’s get to know you"
      left_classes="p-8 pb-0 bg-purple-marketing-300 text-white"
    >
      ...
      <ul class="mb-8 space-y-2">
        <li class="flex gap-2">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          3 months of Todoplace’s all-in-one software
        </li>
        <li class="flex gap-2">
          <.icon name="checkcircle" class="h-4 w-4 flex-shrink-0 mt-1.5" />
          Best-in-the- business customer support
  ...
  defp signup_form(assigns) do
    ...
    <a
      ...
      href={~p"/auth/google"}
      phx-hook="ShowLoader"
      class="flex items-center justify-center w-full mt-8 text-center btn-primary"
    >
      <.icon name="google" width="25" height="24" class="mr-4" /> Continue with Google
    </a>
    ...
    <.form
      ...
      class={"flex flex-col #{@form_classes}"}
    >
      ...
      <p class="text-sm text-gray-400 mt-6 sm:pr-6 mb-8">
