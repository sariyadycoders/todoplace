  ...
  def render(assigns) do
    ...
    <div class="flex flex-col justify-between">
      <div class="items-center mt-8">
        <div class={
          classes("flex items-center pt-4 font-sans lg:text-lg text-2xl font-bold", %{
            "text-gray-400" => @category.coming_soon
          })
        }>
          <%= @category.name %>
        </div>
          ...
          <div class={classes("mt-4 pb-4 bg-gray-200", %{"bg-gray-200/20" => @category.coming_soon})}>
            <div class="flex justify-start pt-4 pl-4">
              <%= if @category.coming_soon do %>
                <button
                  class="p-2 font-bold rounded-lg text-blue-planning-300 bg-blue-planning-100"
                  disabled
                >
                  Coming soon!
                </button>
              <% else %>
                <%= if @product.product_preview_enabled and @product.sell_product_enabled do %>
                  <button
                    class={
                      classes(
                        "flex items-center font-sans text-sm py-2 pr-3.5 pl-3 bg-white border border-blue-planning-300 rounded-lg cursor-pointer",
                        %{
                          "pointer-events-none opacity-30 hover:opacity-30 hover:cursor-not-allowed" =>
                            @disabled
                        }
                      )
                    }
