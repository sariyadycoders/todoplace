defmodule TodoplaceWeb.LayoutView do
  ...
  def subscription_ending_soon(%{current_user: current_user} = assigns) do
    subscription = current_user |> Todoplace.Subscriptions.subscription_ending_soon_info()
  ...
  def stripe_setup_banner(%{current_user: current_user} = assigns) do
    ...
    <%= if !Enum.member?([:charges_enabled, :loading], @stripe_status) do %>
      <div class="bg-gray-100 py-3 border-b border-b-white mt-10">
  ...
  defp load_organization(gallery) do
    gallery
    |> Todoplace.Repo.preload([job: [client: :organization]], force: true)
