...
defmodule Todoplace.Accounts.Notification do
  ...
  schema "notifications" do
    ...
    belongs_to :organization, Todoplace.Accounts.Organization
  ...
  def changeset(notification, attrs) do
    notification
    # Cast the organization_id as well
    |> cast(attrs, [:title, :body, :user_id, :organization_id])
    # Validate that organization_id is required
    |> validate_required([:title, :body, :user_id, :organization_id])
  end

  def send_notification(user_id, organization_id, payload) do
    # Get the FCM token from your storage (e.g., database)
    token =
      "er1H96TkDC1kqVh1Bi75OP:APA91bFK5XxQF8Yw0IbWq0_ua8bkVC1NjLkZDrqwflpEx66MPTZKeRAsuTFPO4My4KonF11nrptmUq_AYA3bK70tgzOsGlJNiyPy_uYpeHcnYwylhWAssVxVYsOTqhGTFom6RkIv8eSD"
    ...
    existing_notification =
      ...
      |> where(
        [n],
        n.user_id == ^user_id and n.body == ^payload.body and
